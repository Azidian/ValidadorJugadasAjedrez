...
class VisualizadorAjedrez:
    ...
    def setup_ui(self):
        num_buttons = 3
        total_width = (num_buttons * 0.15) + ((num_buttons - 1) * 0.05)  # 3 botones + 2 espacios
        start_x = (1 - total_width) / 2  # Centrado horizontal
    ...
    def mostrar_arbol(self):
        print(f"\nCreando interfaz grafica para la partida de ajedrez")
        G = nx.DiGraph()
        ...
        # Dibujar el árbol
        print(f"- Generando Arbol Binario para: {listMovimientos}")
        self.draw_tree(G)
        plt.show()
    ...
    def draw_tree(self, G):
        ...
        # Dibujar nodos
        nx.draw_networkx_nodes(G, self.node_positions, ax=self.ax, node_color=node_colors, 
                               node_size=1500, edgecolors='dimgray', linewidths=1.5)
        ...
        # Configuración final
        self.ax.set_title("Árbol Binario de Partida de Ajedrez",
                 pad=20,
                 fontsize=20,
                 fontweight='bold',
